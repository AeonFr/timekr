  ...
  const importData = (e: React.FormEvent) => {
    ...
    fr.onloadend = (evt) => {
      ...
      try {
        ...
        alert(
          "Error during file evaluation! The file is probably in the incorrect format - JSON.parse failed",
        );
        console.log(e);
        return false;
      }
    ...
    <section className="container font-sans">
      <div className="m:flex flex-row-reverse mb-4">
        ...
        <aside className="w-full m:w-1/3 px-4 mt-8 m:mt-2">
          <Link to="/" className="flex items-center no-underline text-1 mb-3">
            <img src="/assets/logo.png" className="w-8 mr-4" alt="Logo" />
          ...
          {showImportForm && (
            <form className="mt-2 show-ltr" onSubmit={importData}>
              <input
                ref={fileInputRef}
                type="file"
